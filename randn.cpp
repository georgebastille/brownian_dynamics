...
long seed_me(long in_seed)
{
	unsigned long seed;

	// if the seed is not set, use a pseudo random source
    if (in_seed == 0)
    {		
		// seed with kernels PRNG
		ifstream random_source("/dev/urandom");
		random_source.read(reinterpret_cast<char*>(&seed), sizeof(seed));
		srand(seed);
		random_source.close();
	}
	else // otherwise use the defined seed
	{
		srand(in_seed);
		seed = in_seed;
	}
